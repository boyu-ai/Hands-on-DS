...
struct TicketInfo {
  ...
  bool operator==(const TicketInfo &rhs) const {
    return trainID == rhs.trainID && departureStation == rhs.departureStation &&
           arrivalStation == rhs.arrivalStation && seatNum == rhs.seatNum &&
           price == rhs.price && duration == rhs.duration && date == rhs.date;
  }

  bool operator!=(const TicketInfo &rhs) const { return !(*this == rhs); }
  ...
  friend std::ostream &operator<<(
      std::ostream &os, const TicketInfo &ticketInfo) {
