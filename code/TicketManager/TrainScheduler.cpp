...
namespace trainsys {
TrainScheduler::TrainScheduler() { passingStationNum = 0; }
...
void TrainScheduler::insertStation(int i, const StationID &station) {
  for (int j = passingStationNum; j > i; j--) { stations[j] = stations[j - 1]; }
  stations[i] = station;
  passingStationNum++;
}
...
void TrainScheduler::setTrainID(const TrainID &id) { this->trainID = id; }

void TrainScheduler::setPrice(const int price[]) {
  for (int i = 0; i + 1 < passingStationNum; i++) { this->price[i] = price[i]; }
}
...
void TrainScheduler::setSeatNumber(int seatNum) { this->seatNum = seatNum; }

TrainID TrainScheduler::getTrainID() const { return trainID; }

int TrainScheduler::getSeatNum() const { return seatNum; }

int TrainScheduler::getPassingStationNum() const { return passingStationNum; }

StationID TrainScheduler::getStation(int i) const { return stations[i]; }

int TrainScheduler::getDuration(int i) const { return duration[i]; }

int TrainScheduler::getPrice(int i) const { return price[i]; }
...
void TrainScheduler::getPrice(int *price) const {
  ...
  for (int i = 0; i + 1 < passingStationNum; i++) { price[i] = this->price[i]; }
}
...
std::ostream &operator<<(
    std::ostream &os, const TrainScheduler &trainScheduler) {
