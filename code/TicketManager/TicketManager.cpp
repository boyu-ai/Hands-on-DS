// 代码清单8-10到8-12 TicketManager类的实现
...
TicketManager::TicketManager(const std::string &filename)
    : ticketInfo(filename) {}

TicketManager::~TicketManager() {}

int TicketManager::querySeat(
  ...
  for (int i = 0; i < relatedInfo.length(); ++i) {
    if (relatedInfo.visit(i).date == date &&
        relatedInfo.visit(i).departureStation == stationID) {
...
int TicketManager::updateSeat(const TrainID &trainID, const Date &date,
  ...
  return -1;  // 出错，未找到符合条件的余票信息
}

void TicketManager::releaseTicket(
    const TrainScheduler &scheduler, const Date &date) {
